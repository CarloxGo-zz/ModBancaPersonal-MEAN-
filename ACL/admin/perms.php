...
<div id="page">
    ...
    if ( isset($_GET['action']) && $_GET['action'] == 'perm') { 
        ...
        <?php } else { ?>
    	<h2>Manage Permission: (<?= $myACL->getPermNameFromID($_GET['permID']); ?>)</h2><?php } ?>
        <form action="perms.php" method="post">
        	<label for="permName">Name</label> : 
        	<input	type="text" name="permName" id="permName"
        				value="<?php if( isset($_GET['permID']) ){
         								echo $myACL->getPermNameFromID($_GET['permID']);
         							 } ?>"
         			maxlength="30" /><br />
         
         <label for="permKey">Key</label> : 
         <input	type="text" name="permKey" id="permKey"
         			value="<?php if( isset($_GET['permID']) ){
         								echo $myACL->getPermKeyFromID($_GET['permID']);
         							 } ?>"
         			maxlength="30" /><br />
    		
    		<input type="hidden" name="action" value="savePerm" />
        	<input type="hidden" name="permID" value="<?php if(isset($_GET['permID'])){ echo $_GET['permID']; } ?>" />
